...
_DOUBLE_WEAPON_COMBAT_INVENTORY_BUCKETS = {"MAINHAND"}
...
def _tab_dump_equipped_weapon_proc_hooks(h: int, actor_ptr: int) -> dict[str, bool]:
    """Return unmodeled proc-hook flags from equipped weapon combat tables."""
    ...
    for bucket_ptr in _tab_get_ordered_tables(h, inven_tab):
        ...
        if bucket_name not in _WEAPON_COMBAT_INVENTORY_BUCKETS:
            continue
        for item_ptr in _tab_get_ordered_tables(h, bucket_ptr):
            for combat_key in ("combat", "special_combat"):
                combat_tab = _tab_get_table(h, item_ptr, combat_key)
                if combat_tab is None:
                    continue
                for hook in _COMBAT_PROC_HOOK_TABLES:
                    hook_tab = _tab_get_table(h, combat_tab, hook)
                    if hook_tab and _tab_has_any_entries(h, hook_tab):
                        out[f"combat.{hook}"] = True
    return out
...
def _offhand_combat_prefix(index: int) -> str:
    return "combat.offhand." if index == 1 else f"combat.offhand{index}."


def _tab_is_archery_item(h: int, item_ptr: int) -> bool:
    return _tab_get_table(h, item_ptr, "archery") is not None or bool(_tab_get_scalar(h, item_ptr, "archery"))


def _tab_dump_one_offhand_combat(
    h: int,
    combat_tab: int,
    *,
    prefix: str,
    ...
    fields = _with_combat_prefix(_tab_dump_combat_fields(h, combat_tab), prefix)
    if not fields:
        return {}
    no_penalty = bool(_tab_get_scalar(h, combat_tab, "no_offhand_penalty"))
    fields[f"{prefix}mult"] = 1.0 if no_penalty else 0.5
    fields[f"{prefix}source"] = source
    return fields


def _tab_dump_offhand_weapon_combat_fields(h: int, actor_ptr: int) -> dict[str, str | float | bool]:
    """Return combat fields for equipped offhand melee weapons."""
    inven_tab = _tab_get_table(h, actor_ptr, "inven")
    if inven_tab is None:
        return {}

    out: dict[str, str | float | bool] = {}
    double_weapon_item: int | None = None

    for bucket_ptr in _tab_get_ordered_tables(h, inven_tab):
        bucket = _tab_dump_flat(h, bucket_ptr, allowed_keys={"name", "short_name"})
        bucket_name = str(bucket.get("short_name") or bucket.get("name") or "").upper()
        if bucket_name not in _DOUBLE_WEAPON_COMBAT_INVENTORY_BUCKETS:
            continue
        for item_ptr in _tab_get_ordered_tables(h, bucket_ptr):
            if double_weapon_item is not None or not _tab_get_scalar(h, item_ptr, "double_weapon"):
                continue
            if _tab_is_archery_item(h, item_ptr):
                continue
            double_weapon_item = item_ptr
    ...
    for bucket_ptr in _tab_get_ordered_tables(h, inven_tab):
        ...
        for item_ptr in _tab_get_ordered_tables(h, bucket_ptr):
            ...
            if combat_tab is None:
                continue
            source = bucket_name if index == 1 else f"{bucket_name} #{index}"
            fields = _tab_dump_one_offhand_combat(
                h,
                combat_tab,
                prefix=_offhand_combat_prefix(index),
                source=source,
            )
            if fields:
                out.update(fields)
    ...
    if out or double_weapon_item is None:
        return out

    combat_tab = _tab_get_table(h, double_weapon_item, "combat")
    if combat_tab is None:
        return {}
    return _tab_dump_one_offhand_combat(
        h,
        combat_tab,
        prefix=_offhand_combat_prefix(1),
        source="DOUBLE_WEAPON",
    )
